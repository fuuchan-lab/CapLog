# CapLog Android（Play Store 用 TWA パッケージ）

Bubblewrap で作った Trusted Web Activity。アプリ本体は https://caplog.doitmyself.net/ の PWA で、
...
$env:Path = "$env:JAVA_HOME\bin;C:\Program Files\nodejs;C:\Users\user\CapLog\android;$env:Path"   # android を PATH に足すのは gradlew.bat を見つけさせるため
$pw = (Get-Content C:\Users\user\.bubblewrap\keys\caplog-keystore-password.txt -Raw).Trim()
$env:BUBBLEWRAP_KEYSTORE_PASSWORD = $pw; $env:BUBBLEWRAP_KEY_PASSWORD = $pw
cd C:\Users\user\CapLog\android
...
## 署名鍵（アップロード鍵）
- ファイル: C:\Users\user\.bubblewrap\keys\caplog-upload.keystore（別名 caplog）、パスワードは同じフォルダーの txt
