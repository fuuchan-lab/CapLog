...
const translations = {
  ja: {
    ...
    exportHelp: '記録を Excel ファイル・写真・データ（JSON）にまとめて書き出します。Google Drive の CapLog フォルダーの中に書き出しごとのフォルダーを作って保存するか、この端末に ZIP でダウンロードできます。',
  ...
  en: {
    ...
    exportHelp: 'Bundles your records as an Excel file, photos and data (JSON). Save it to a new folder inside the CapLog folder in Google Drive, or download it to this device as a ZIP.',
...
function loadThemePreference() {
  try {
    const saved = localStorage.getItem('caplog-theme');
...
themeSelect.addEventListener('change', () => {
  ...
  try {
    if (preference === 'auto') localStorage.removeItem('caplog-theme');
    else localStorage.setItem('caplog-theme', preference);
  } catch {
...
const DEVICE_ID_KEY = 'caplog-device-id';
